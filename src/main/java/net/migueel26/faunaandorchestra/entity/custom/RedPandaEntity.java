...
public class RedPandaEntity extends MusicalEntity {
    ...
    public void onEat(ItemEntity targetEntity) {
        ...
        if (stack.is(ModItems.PERFUMED_BAMBOO.get())) {
    ...
    public boolean isFood(ItemStack stack) {
        return stack.is(Items.BAMBOO) || stack.is(ModItems.PERFUMED_BAMBOO.get());
    }
    ...
    public EntityDimensions getDimensions(Pose pose) {
        return this.isBaby() ? ModEntities.RED_PANDA.get().getDimensions().scale(0.7f) : super.getDimensions(pose);
    }
    ...
    public void playSpecialClothingAnimation(ItemStack stack) {
        if (stack.is(ModItems.IMAGINAL_DISK.get())) {
    ...
    private class RedPandaEatingBambooGoal extends Goal {
        ...
        public void stop() {
            redPanda.setEating(false);
            redPanda.spawnAtLocation(ModItems.SHARP_BAMBOO.get(), 1);
