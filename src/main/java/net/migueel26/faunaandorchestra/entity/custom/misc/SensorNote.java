...
public class SensorNote extends Entity {
    private static final EntityDataAccessor<Integer> TEXTURE_INDEX =
            SynchedEntityData.defineId(SensorNote.class, EntityDataSerializers.INT);
    private double tx;
    private double ty;
    private double tz;
    private int life;

    public SensorNote(EntityType<? extends SensorNote> entityType, Level level) {
        super(entityType, level);
    }

    public SensorNote(Level level, double x, double y, double z) {
        this(ModEntities.SENSOR_NOTE.get(), level);
        this.setPos(x, y, z);
    }
    ...
    public void lerpMotion(double x, double y, double z) {
        ...
        if (this.xRotO == 0.0F && this.yRotO == 0.0F) {
            ...
            this.setYRot((float)(Mth.atan2(x, z) * 180.0 / 3.1415927410125732));
            this.setXRot((float)(Mth.atan2(y, d0) * 180.0 / 3.1415927410125732));
    ...
    public void tick() {
        ...
        this.setXRot(lerpRotation(this.xRotO, (float)(Mth.atan2(vec3.y, d3) * 180.0 / 3.1415927410125732)));
        ...
        if (!this.level().isClientSide) {
            ...
            vec3 = new Vec3(Math.cos((double)f1) * d6, d7 + ((double)j - d7) * 0.014999999664723873, Math.sin((double)f1) * d6);
        ...
        if (this.isInWater()) {
            ...
            this.level().addParticle(ParticleTypes.FLAME, d0 - vec3.x * 0.25 + this.random.nextDouble() * 0.6 - 0.3, d1 - vec3.y * 0.25 - 0.5, d2 - vec3.z * 0.25 + this.random.nextDouble() * 0.6 - 0.3, vec3.x, vec3.y, vec3.z);
        }
        ...
        if (!this.level().isClientSide) {
            ...
            if (this.life > 80 && !this.level().isClientSide) {
                ...
                this.discard();
                ((ServerLevel) level()).sendParticles(ModParticleTypes.REGULAR_NOTE.get(), position().x, position().y, position().z, 15, 0.2, 0.2, 0.2, 0.1);
            }
        } else {
